...
\begin{itemize}
	...
	\begin{itemize}
		\item S21 - Oś 2, podpora 1, kierunek X,
		\item S22 - Oś 1, podpora 1, kierunek Y,
		\item S23 - Oś 1, podpora 2, kierunek X,
		\item S24 - Oś 2, podpora 2, kierunek X,
		\item S25 - Oś 1, podpora 1, kierunek X,
		\item S26 - Oś 2, podpora 1, kierunek Y,
		\item S27 - Oś 1, podpora 2, kierunek Y,
		\item S28 - Oś 2, podpora 2, kierunek Y,
	\end{itemize}
...
Kalibrację modelu postanowiono potraktować jako problem optymalizacji. Problem zdefiniowano klasycznie przez wybór funkcji celu, parametrów i zmiennych projektowych oraz ograniczeń. Funkcję celu naturalnie jest miara dopasowania modelu do konstrukcji rzeczywistej pod względem charakterystyk dynamicznych. Parametry projektowe definiują model numeryczny poddawany kalibracji i zostały przyjęte na podstawie dokumentacji powykonawczej. Zmienne projektowe stanowią wymienione wyżej 22 parametry: sztywności elementów konstrukcji, warunków brzegowych i masy znajdujące się na obiekcie. Ograniczenia dla zmiennych projektowych stanowią wartości graniczne podyktowane doświadczeniem i zdrowym rozsądkiem. Niedopuszczalne i niemiarodajne byłoby przykładowo umożliwienie zmian sztywności wszystkich elementów stalowych o $\pm$50\%. Przykładowe niepewności dla różnych elementów modelu numerycznego przedstawiono w tabeli \ref{table:uncertainitesModel}. Jako metodę optymalizacji przyjęto metodę roju cząstek PSO. 
...
\end{equation}
gdzie $\text{MAC}(\cdot)$ oznacza kryterium MAC dwóch wektorów. Zasadniczo kryterium MAC dwóch wektorów powinno być maksymalizowane. Dla idealnego dopasowania wektorów równe jest jedności, a dla zupełnego braku dopasowania - zeru. Znając wartość maksymalną kryterium MA0C i stosując odejmowanie przemianowano naturalnie maksymalizowaną funkcję celu na minimalizowaną. Dzięki temu wszystkie funkcje celu będą podlegać minimalizacji co uprasza interpretację wyników oraz zmniejsza złożoność algorytmu obliczeniowego.
