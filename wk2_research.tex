...
\end{itemize}
które stanowią zmienne projektowe problemu optymalizacji $\vect{X}$. Zmienne zaznaczono na przekrojach poprzecznych przedstawionych na rysunku \ref{fig:wk2_opti_cs_elements}.
\begin{figure}[h]
	\centering
...
W procesie optymalizacji uwzględniono również ograniczenia i wymagania jakie musi spełnić rozwiązanie, aby zostało uznane jako poprawne. Sprawdzane wartości zestawiono w wektorze $\vect{C}$. Bazując na obowiązujących przepisach wyznaczono dla każdego przypadku:
\begin{itemize}
	\item normowe, maksymalne, obliczeniowe naprężenia w dźwigarze łukowym $c_{sa}$, w ściągu $c_{sg}$ i w wieszakach $c_{sh}$ pod obciążeniem statycznym,
...
Prędkości pociągów przyjmowano zgodnie z zaleceniem w zakresie od 40 km/h do maksymalnej prędkości obliczeniowej ($1.2\times\,$nominalna prędkość maksymalna). Nie przyjmowano stałego kroku uwzględnianych prędkości. Wybrano jedynie prędkości które mieszczą się w powyższym zakresie oraz są prędkościami rezonansowymi. Prędkości rezonansowe obliczano na podstawie wzoru \ref{}. Potrzebne częstotliwości drgań własnych odczytywano z modelu numerycznego po przeprowadzeniu analizy modalnej. Dla każdego pociągu HSLM-A wyznaczano prędkość na bazie typowego rozstawu osi $d$, odczytanych częstotliwości drgań własnych $\phi_i$ oraz przyjmując wartość $n = 1,2\dots 10$. Jeżeli prędkość rezonansowa dla danego pociągu mieściła się w wymaganym zakresie dodawano ją do obliczanego zestawu. 
...
Optymalizację przeprowadzono ponownie za pomocą algorytmu MOPSO. Ze względu na uniwersalność algorytmów metaheurystycznych w wielu aspektach tok postępowania jest zbliżony do zastosowanego w procesie kalibracji. Wyróżniono następujące główne kroki wykonywane dla każdego nowego położenia cząstki:
\begin{enumerate}
