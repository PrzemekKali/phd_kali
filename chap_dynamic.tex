...
\subsection{Obciążenie dynamiczne mostów kolejowych}
...
\begin{equation} \label{eq:critical_speed_2}
	...
	\end{matrix*}
\end{equation}
%Częstotliwość pojawiania się kolejnych osi jest oczywiście związana z prędkością. Niemniej \cite{Fryba2001} podaje również drugą formułę na prędkość krytyczną, w której zakłada się dla małego tłumienia równość $\omega_j^2 = j^2 \omega^2$, gdzie $\omega_j$ oznacza częstość drgań własnych konstrukcji, a $\omega = \frac{\pi c}{l}$, gdzie $c$ jest prędkością przejazdu, a $l$ rozpiętością przęsła. Formuła na drugi rodzaj prędkości krytycznej jest następująca:
%\begin{equation}
...
Nierównomierności cykliczne opisane mogą być za pomocą szeregów Fouriera \parencite{Ladislav1996}. Rozwiązanie to pozwala również na opisanie jako nierównomierności toru pokrewnych, rzeczywistych efektów takich jak efekt wypłaszczenia powierzchni tocznej obręczy kół \parencite{Zhou2020} czy efekt zmiany sztywności jezdni na poprzecznicach i podkładach w pomoście z jezdnią otwartą \parencite{Fryba1999}. Z drugiej strony, nierównomierności losowe najczęściej są efektem źle wykonanego lub źle utrzymywanego toru. Powyższe oddziaływania wywołują z reguły pionowe drgania. Z kolei poprzeczne wzbudzenia związane mogą być z podłużnymi lub poprzecznymi siłami. Pierwsze z nich wynikają głównie z przyspieszenia bądź hamowania taboru na moście. Drgania poprzeczne związane są głównie z bocznymi ruchami pojazdu. Mają one swoje źródło w nierównomiernościach poprzecznych toru, mechanizmie wężykowania \parencite{Babe2016} i występowaniu siły odśrodkowej. Na rysunku \ref{} przedstawiono schemat podsumowujące rzeczywiste oddziaływania \parencite{Ladislav1996}.
