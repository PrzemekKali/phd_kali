...
Dla wybranych rozwiązań kompromisowych stworzono rozkłady zmiennych projektowych. Na rysunku \ref{fig: calibration_histogramy} przestawiono znormalizowane histogramy wskazujące udział zmiennej w danym wąskim przedziale wartości wśród wszystkich rozwiązań kompromisowych. Niebieskim kolorem oznaczono histogramy dotyczące sztywności, pomarańczowym podpór sprężystych, a zielonym mas. Wąskie rozkłady wskazują, że zmienna osiągała zbliżone wartości we wszystkich rozwiązaniach kompromisowych. Oznacza to również, że jej wpływ jest bardzo istotny na osiągnięcie akceptowalnych wartości funkcji celu i jego zmiana powoduje wykluczenie z zaakceptowanego zakresu. Z drugiej strony, szerokie rozkłady z dużym rozproszeniem wartości wskazują na mały wpływ danej zmiennej na rezultaty funkcji celu. 

Z przedstawionych rezultatów można odczytać, że zmiana sztywności łuku, ściągu, poprzecznic, stężeń i wezgłowi w zadanych zakresach jest niewielka. Jest to zgodne z doświadczeniem i zdrowym rozsądkiem. Są to główne elementy konstrukcyjne i ich znaczne modyfikacje wywołują istotne zmiany w pracy konstrukcji. Należy zwrócić uwagę, że według kalibracji wymodelowany łuk powinien być mniej sztywny o około 8\% niż to wymodelowano początkowo, a z kolei elementy pomostu: ściąg i poprzecznice sztywniejsze o około 10\%. Większość rozwiązań przewiduje stężenia o sztywności bliskiej zamodelowanej. Model wezgłowi opisanych jak w punkcie \ref{sect:wk2_basic_num_model} większość rozwiązań proponuje usztywnić nawet około 8 razy. 

Podpory sprężyste na kierunkach podłużnych i poprzecznych wstępnie zostały przyjęte jako dość sztywne o stałej $k=1\mathrm{E}6\;\mathrm{kN/m}$ co odpowiada wartości 0 na histogramie. Przypomnijmy że wartość na osi poziomej histogramu odpowiada wykładnikowi mnożnika w postaci $10^x$. Wynika stąd, że wartości dodatnie zwiększają, a ujemne zmniejszają sztywność podpory względem wartości wstępnej. Wykładniki mnożników dla niektórych podpór zostały przyjęte wyłącznie jako dodatnie, co można utożsamiać z praktyczną blokadą przemieszczeń na danym kierunku. Dotyczy to głównie podpór 2AX, 1AY, 2BX i 2BY. Z kolei podpory 1BX oraz 1BY wyraźnie są osłabiane co prowadzi do zwolnienia więzów na danym kierunku. Podpory 1AX i 2AY nie uległy znaczącej zmianie w trakcie kalibracji. Porównanie uzyskanych rezultatów z projektowanym schematem łożyskowania pokazuje, że \ref{}.
...
Wszystkie opisane wyniki zostały podsumowane i sformułowano zestaw zmiennych projektowych najczęściej pojawiającym się w rezultatach. Przyjęto mnożniki o wartościach zebranych w tabeli \ref{} dzięki czemu uzyskano zestaw funkcji celu równy: 
