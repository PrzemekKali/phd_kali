...
Piąty zestaw prezentuje zależność między wartością zmiennej projektowej rozwiązania, a uzyskanymi maksymalnymi wartościami przyspieszeń rozwiązań należących do Frontu Pareto. Dla prezentowanych zależności wyznaczono wartości funkcji korelacji \teng{correlation}, cząstkowej korelacji \teng{partial correlation} i semi-czątkowej korelacji \teng{semi-partial correlation} \parencite{}. Wyniki korelacji zestawiono w tabeli (na przykład tab. \ref{}). Do wyznaczania współczynników użyto metody Pearsona. Oceniając wyniki należy zdawać sobie sprawę z nie spełnienia wszystkich założeń wymaganych przy obliczaniu korelacji Pearsona\footnote{Opisać założenia.}. Z tego względu jego wskazania zostały potraktowane jedynie jako wskazówka do poszukiwania zależności pomiędzy zmiennymi niezależnymi i zmienną zależną. W ogólności współczynnik korelacji liniowej $r^2$ wskazuje ... stopień liniowej zależności pomiędzy zmiennymi, współczynnik częściowej korelacji $\delta r^2$ pokazuje, a współczynnik semi-korelacji wskazuje unikalny wpływy danej zmiennej.

Dla każdego wariantu optymalizacji oceniono wyniki i wyciągnięto następujące wnioski dotyczące rozwiązania:
\begin{itemize}
	...
	\item wpływ poszczególnych zmiennych projektowych na maksymalne przyspieszenia obiektu,
\end{itemize} 
