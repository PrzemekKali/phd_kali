...
Dla wybranych rozwiązań kompromisowych sprawdzono rozkłady zmiennych projektowych. Na rysunku \ref{fig: calibration_histogramy} przestawiono znormalizowane histogramy wskazujące udział zmiennej w danym wąskim przedziale wartości wśród wszystkich rozwiązań kompromisowych. Niebieskim kolorem oznaczono histogramy dotyczące sztywności, pomarańczowym podpór sprężystych, a zielonym mas. Wąskie rozkłady wskazują, że zmienna osiągała zbliżone wartości we wszystkich rozwiązaniach kompromisowych. Oznacza to również, że jej wpływ jest bardzo istotny na osiągnięcie akceptowalnych wartości funkcji celu i jego zmiana powoduje wykluczenie z zaakceptowanego zakresu. Z drugiej strony, szerokie rozkłady z dużym rozproszeniem wartości wskazują na mały wpływ danej zmiennej na rezultaty funkcji celu. 

Z przedstawionych rezultatów można odczytać, że zmiana sztywności łuku, ściągu, stężeń i wezgłowi w zadanych zakresach jest niewielka. Jest to zgodne z doświadczeniem i zdrowym rozsądkiem. Są to główne elementy konstrukcyjne i ich znaczne modyfikacje wywołują istotne zmiany w pracy konstrukcji. Należy zwrócić uwagę, że według kalibracji wymodelowany łuk powinien być mniej sztywny o około 8\% niż to wymodelowano początkowo, a ściąg sztywniejszy.
