...
Z tego względu jego wskazania zostały potraktowane jedynie jako wskazówka do poszukiwania zależności pomiędzy zmiennymi i należy je zawsze rozpatrywać w parze z wykresami zależności przyspieszeń od zmiennych \parencite{Vargha2013}. W idealnych warunkach współczynnik korelacji liniowej $r^2$ wskazuje natężenie współzależności dwóch zmiennych \parencite{Czaja2000}. Z kolei współczynnik częściowej korelacji $\rho r^2$, oraz współczynnik semikorelacji $sr^2$ wskazują natężenie współzależności dwóch zmiennych przy uwzględnieniu (wyeliminowaniu) wpływu pozostałych. Korelacja cząstkowa eliminuje wpływ pozostałych zmiennych niezależnych z rozpatrywanej zmiennej zależnej jak i niezależnej. Korelacja semicząstkowa eliminuje wpływ pozostałych zmiennych jedynie ze zmiennej niezależnej. Z tego powodu to semicząstkowa korelacja obrazuje indywidualny wkład poszczególnych zmiennych niezależnych na wariancję zmiennej zależnej. \cite{Tabachnick} wskazuje, że to korelacja semiczastkowa jest lepszą niż cząstkowa miarą istotności wpływu danej zmiennej niezależnej na zmienną zależną.
...
Wykonane analizy optymalizacyjne obiektu łukowego pozwaliły na zauważenie pewnych prawidłowości pomiędzy wymiarami elementów konstrukcyjnych, a zachowaniem dynamicznym obiektu. Skondensowane rezultaty dla 3 wariantów rozmieszczenia wieszaków, dla trzech różnych prędkości maksymalnych przejazdu przedstawiono w tabeli \ref{tab:summary_optimization}. W tabeli zebrano kilka cech charakteryzujących uzyskane w danym wariancie wyniki. Po pierwsze wskazano jaki jest minimalny stopień wykorzystania wytrzymałości elementów $\rho_{min}$, dla rozwiązań optymalnych przy spełnionym warunku maksymalnych przyspieszeń pomostu $a_{max}<3.5\;\mathrm{m/s^2}$. Po drugie dla każdego z wariantów określono przedziały wymiarów elementów, dla których uzyskano wyniki optymalne przy spełnieniu warunku $a_{max}<3.5\;\mathrm{m/s^2}$. W niektórych przypadkach wymiar nie był ograniczony w rozpatrywanym zakresie i wskazanie było niemożliwe. Po trzecie na podstawie analizy wizualnej rezultatów i wskazówek miar korelacji zasugerowano element, którego zmiana najbardziej wpływa na spadek lub wzrost przyspieszeń maksymalnych. Miary korelacji traktowano wyłącznie jako wskazówki i nie przywiązywano do nich kluczowej wagi ze względu na brak spełnienia wszystkich założeń. Pomimo to w większości przypadków wskazywały one te same zalezności co obserwacja wizualna. 
...
\begin{table}[H]
	...
	\resizebox{\textwidth}{!}{%
		\begin{tabular}{@{}ccccccc@{}}
			...
			\multicolumn{3}{c}{Wymiary elementów dla rozwiązań optymalnych} &
