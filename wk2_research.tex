...
Zaprezentowane wyniki wskazują, że dla rozwiązania z dominującym dźwigarem/ściągiem i o wiotkim łuku zmiana wieszaków z układu prostego na Network nie obniża maksymalnych przyspieszeń. Co więcej dla żadnej średnicy ekwiwalentnej nie znaleziono rozwiązania dopuszczalnego. Najmniejsze wartości przyspieszeń uzyskano dla zakresu średnicy ekwiwalentnej od 8 do 16 mm. Zauważyć można również, że po przekroczeniu tych wartości, im sztywniejsze jest połączenie pomiędzy łukiem, a ściągiem tym większe są maksymalne przyspieszenia. Przebieg zmienności maksymalnych przyspieszeń ma charakter raczej gładki, bez nagłych wzrostów wartości.
...
Oba przeanalizowane przypadki pokazują, że istnieje wyraźna wartość graniczna średnicy ekwiwalentnej wieszaków po której przekroczeniu najniższe częstotliwości drgań własnych stabilizują się. Rozpatrując przykładowe przypadki, można zauważyć że stabilizacja dwóch najniższych częstotliwości drgań własnych wydaje się mieć związek ze stabilizacją maksymalnych przyspieszeń pomostu. Dla wariantu z wieszakami prostymi krzywa maksymalnych przyspieszeń staje się niemal płaska od średnicy równej około 46 mm, a dla układu Network od około 70 mm. Układ Network wymaga wprowadzenia większych średnic wieszaków, ponieważ łuk odgrywa znacznie większą rolę w przenoszeniu obciążeń i jest znacznie sztywniejszy niż w przypadku niskiego przekroju łuku w wariancie z wieszakami prostymi. Rozpatrywane przykłady zmiany sztywności układu i przez to częstotliwości drgań własnych są przypadkiem szczególnym i nie pozwalają na wyciągnięcie ogólnych wniosków. W kolejnym kroku zdecydowano o przeanalizowaniu całych zestawów rozwiązań pochodzących z optymalizacji w przestrzeni częstotliwości drgań własnych.

\subsection{Rezultaty w dziedzinie pierwszych dwóch modów}

%\subsubsection{Podział ze względu na postaci drgań własnych}

Na podstawie zauważonej współzależności stabilizacji przyspieszeń maksymalnych i pierwszych częstotliwości drgań własnych układu, w niniejszej części pracy wartości funkcji celu oraz zmiennych projektowych przedstawiono w zależności od pierwszych dwóch częstotliwości drgań własnych. Jednak tym razem rozróżniono mody również ze względu na postaci drgań własnych. Rozpatrywano dwie pierwsze postaci drgań własnych, z których jedna zawsze charakteryzuje się rozkładem przemieszczeń w formie zbliżonej do jednej półfali sinusa, a druga do dwóch półfal sinusa. Do identyfikacji postaci użyto kryterium MAC. Odczytano postaci drgań (kilkadziesiąt punktów dla łuku i ściągu) dla wszystkich rozwiązań i zestawiono z wzorcami o klarownych postaciach z jedną lub dwiema półfalami. Rezultaty przedstawiono na rysunku \ref{fig:minimum_mac_values_shape_division}.

\begin{figure}[hbt!]
	\centering
	\subfloat[wieszaki proste]{\includegraphics[width=0.47\textwidth,trim=40 20 5 25, clip]{WK2/opti/proste_160/proste_160_FREQ_MIN_MAC_XY.pdf}}
	\subfloat[wieszaki ukośne]{\includegraphics[width=0.47\textwidth,trim=40 20 5 25, clip]{WK2/opti/niels_160/niels_160_FREQ_MIN_MAC_XY.pdf}} \\
	\subfloat[wieszaki Network]{\includegraphics[width=0.47\textwidth,trim=40 20 5 25, clip]{WK2/opti/netw_160/netw_160_FREQ_MIN_MAC_XY.pdf}}
...
\begin{itemize}
	...
	\item (h) dzielnik prędkości krytycznej, wywołującej maksymalne przyspieszenia pomostu. W przypadku decydujacej prędkości maksymalnej przyjęto 0.
\end{itemize}
...
Na rysunkach (a-d) zielonym prostokątem zaznaczono obszar zawierający wszystkie rozwiązania z przyspieszeniami maksymalnymi pomostu mniejszymi niż $3.5\;\mathrm{m/s^2}$. Czerwony trójkąt oznacza \enquote{najlepsze} rozwiązanie - minimalna masa konstrukcji i jednocześnie przyspieszenia spełniające warunek $3.5\;\mathrm{m/s^2}$ (Tablica \ref{tab:summary_optimal_results}).

Na rysunkach (e-h) zaznaczono czarnymi i czerwonymi liniami przerywanymi pewne charakterystyczne częstotliwości $f_{v,k}^{min/max}$ (przykład pokazano na rysunku \ref{fig:opti_res_2_freqs_example}). Obliczono je przyrównując prędkość maksymalną przejazdu $v_{max}$ do prędkości krytycznej $c_{cr}$. Powyższą równość obliczono dla następujących modeli obciążenia:
\begin{itemize}
...
\end{itemize}  
i dzielników prędkości krytycznej równych $k = 1, 2, 3\;\mathrm{lub}\;4$ według wzoru (\ref{eq:critical_speed_2}). Przykładowo dla prędkości maksymalnej $v_{max}=160\,\mathrm{km/h}$ i dzielnika $k=1$ otrzymano następujące częstotliwości:
\begin{equation*}
	\begin{split}
...
\begin{figure}[hbt!]
	\centering
	\includegraphics[width=0.5\textwidth,trim=40 20 5 20, clip]{WK2/opti/proste_200/proste_200_FREQ_VELOCITY_XY.pdf} 
	\caption{Przykładowy rysunek z zaznaczonymi częstotliwościami $f_{v,k}^{min/max}$}
