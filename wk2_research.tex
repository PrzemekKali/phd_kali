...
Wynika stąd, że wariant z wieszakami prostymi po zmianie na wieszaki Network nie powinien spełnić warunków poprawności rozwiązania. W przeciwnym razie ujawniłby się jako rozwiązanie lepsze (lżejsze) od wyznaczonych rozwiązań optymalnych dla wariantu Network. Dokonano eksperymentu i sprawdzono maksymalne przyspieszania dla \enquote{najlepszego} rozwiązania z wieszakami prostymi, jedynie zamieniając układ wieszaków na wariant Network. Maksymalne przyspieszenia pomostu wyniosły w tym przypadku ponad 4.00 $\text{m}/\text{s}^2$.

Poszukiwania przyczyn zaistniałego dysonansu przeniesiono do dziedziny częstotliwości drgań własnych układu. Dla \enquote{najlepszego} optymalnego zestawu wymiarów elementów konstrukcyjnych wariantu z wieszakami prostymi i przy prędkości maksymalnej 300 km/h (Tab. \ref{tab:summary_optimal_results}) przeprowadzono eksperyment polegający na zamianie wieszaków na system Network i uzmiennieniu sztywności wieszaków. Sztywność ta ograniczona jest jedynie do sztywności podłużnej zdefiniowanej iloczynem modułu sprężystości i pola przekroju wieszaków ($EA$). Nie chcąc uwzględniać wpływu zmiany masy w prowadzonej analizie przyjęto stałą średnicę wieszaków jak w \enquote{najlepszym} rozwiązaniu dla wieszaków prostych. Jako zmienną przyjęto moduł sprężystości materiału. Dla każdego przypadku sztywności wieszaków wyznaczono przyspieszenia maksymalne zgodnie z algorytmem opisanym w punkcie \ref{sect:optimization_assumptions}. Obliczono również sześć pierwszych częstotliwości drgań własnych układu. Wyniki zaprezentowano w zależności od ekwiwalentnej średnicy. Ekwiwalentna średnica wyznacza pole przekroju kołowego tak, aby wartość sztywności $EA$ dla danego zmienionego modułu sprężystości odpowiadała modułowi sprężystości stali $E=210 \mathrm{GPa}$. Rezultaty przedstawiono na rysunku \ref{fig:proste_na_network_change_hangers_stiff}. 
