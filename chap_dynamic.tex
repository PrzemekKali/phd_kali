...
Jeżeli analiza dynamiczna jest wymagana norma nakazuje wyznaczenie przyspieszeń przęsła oraz współczynnika $\phi'_dyn$ zdefiniowanego równaniem (\ref{eq:phi_dyn}) \ref{KLASZTORNY} - jakie efekty ?. Do obliczeń należy zastosować obciążenia Pociągami Rzeczywistymi występującymi na linii oraz modele HSLM jeśli dla linii stosowane są międzyoperacyjne kryteria europejskie dużych prędkości.
\begin{equation} \label{eq:phi_dyn}
	\phi '_{dyn} = \text{max}(\frac{y_{dyn}}{y_{stat}})-1
\end{equation}
gdzie: $y_{dyn}$ oznacza maksymalną odpowiedź dynamiczną, a $y_{stat}$ odpowiadającą maksymalną odpowiedź statyczną od obciążenia Pociągiem Rzeczywistym lub modelem HSLM. Prędkości analizowanego przejazdu należy przyjmować od 40 km/h do maksymalnej prędkości obliczeniowej co 10 km/h. Maksymalną prędkość obliczeniową zaleca się przyjmować jako $1.2\times\,$maksymalna dopuszczalna prędkość pojazdu. W pobliżu prędkości rezonansowych (\ref{eq:criticla_speed}) należy zagęścić obliczenia. Według przepisów \parencite{PKNj,UnionInternationaleDesCheminsDeFer2009} przyspieszenia powinny być wyznaczone w zakresie częstotliwości sygnału do $f_{max}=30\,\text{Hz}$. Ma to swoje potwierdzenie w badaniach nad destabilizacją podsypki \parencite{Zacher2008}.

Po wykonaniu analiz należy zweryfikować niebezpieczństwo wystąpienia nadmiernych drgań w konstrukcji. 
