...
Czwarty zestaw prezentuje zależność między wartościami jednej zmiennej projektowej, a uzyskanymi maksymalnymi wartościami przyspieszeń dla rozwiązań należących do Frontu Pareto (na przykład rys. \ref{fig:EXAMPLE_picture_4_5_a}). Dla prezentowanych zależności wyznaczono wartości funkcji korelacji zerowej \teng{correlation}, korelacji cząstkowej \teng{partial correlation} i korelacji semicząstkowej (częściowej) \teng{semipartial correlation} \parencite{Tabachnick}. Wyniki korelacji zestawiono w tabelach (na przykład tab. \ref{tab: correlation_proste_160}). Do wyznaczania współczynników użyto metody Pearsona. Oceniając wyznaczone miary jakości dopasowania należy zdawać sobie sprawę ze spełnienia jedynie części założeń wymaganych przy obliczaniu korelacji Pearsona\footnote{
...
\begin{figure}[p]
	\centering
	\subfloat[wieszaki proste,\\ $v_{max} = 160$ km/h]{\includegraphics[trim= 07  5 05 05, clip]{WK2/opti/proste_160/proste_160_diffs.pdf}} \hfill
	\subfloat[wieszaki proste,\\ $v_{max} = 200$ km/h]{\includegraphics[trim= 20  5 05 05, clip]{WK2/opti/proste_200/proste_200_diffs.pdf}} \hfill
	\subfloat[wieszaki proste,\\ $v_{max} = 300$ km/h]{\includegraphics[trim= 20  5 05 05, clip]{WK2/opti/proste_300/proste_300_diffs.pdf}}\\
...
\begin{figure}[hbt!]
	\centering
	\subfloat[wieszaki proste]{\includegraphics[width=0.47\textwidth,trim=40 27 5 27, clip]{WK2/opti/proste_160/proste_160_FREQ_MIN_MAC_XY.pdf}}
...
\begin{figure}[hbt!]
	...
	\caption{Przykładowy rezultat z zaznaczonymi częstotliwościami $f_{v,k}^{min/max}$: \\ wieszaki proste, maksymalna dopuszczalna prędkość $v_{max}=200\;\mathrm{km/h}$}
...
\begin{enumerate}
	...
	\end{itemize}
	\item Maksymalne przyspieszenia charakteryzują się skokowymi zmianami związanymi z wielkością częstotliwości drgań własnych o podwójnej półfali (rys. \ref{fig:opti_res_2_freqs_prost_160_b}). Granice te pokrywają się z częstotliwościami $f_{160,1}^{min}$ i $f_{160,1}^{max}$ postaci z podwójną półfalą. 
...
\begin{enumerate}
	\item Maksymalne przyspieszenia charakteryzują się skokowymi zmianami dla częstotliwości drgań własnych o podwójnej półfali (rys. \ref{fig:opti_res_2_freqs_prost_200_b}). Granice te pokrywają się z częstotliwościami $f_{200,1}^{min}$ i $f_{200,1}^{max}$ postaci z podwójną półfalą. 
...
\begin{enumerate}
	\item Maksymalne przyspieszenia charakteryzują się skokowymi zmianami dla częstotliwości drgań własnych o podwójnej półfali (rys. \ref{fig:opti_res_2_freqs_prost_300_b}). Granice te pokrywają się z częstotliwościami $f_{300,1}^{min}$ i $f_{300,1}^{max}$ postaci z podwójną półfalą. 
...
\begin{enumerate}
	...
	\item Korelacja pomiędzy wysokością ściągu, a częstotliwością drgań własnych o postaci z podwójną półfalą istnieje, ale jest znacznie słabsza niż w przypadku wieszaków prostych (rys. \ref{fig:opti_res_2_freqs_niels_160_c}).
...
\item Niemal wszystkie rozwiązania mieszczą się powyżej granicy $f_{200,1}^{max}$ dla częstotliwości o obu rozpatrywanych postaciach. Przekłada się to na konieczność zastosowania dzielnika prędkości krytycznej $k>1$ (rys. \ref{fig:opti_res_2_freqs_niels_200_h}).
...
\item Przyspieszenia dopuszczalne $a_{max}<3.5\;\mathrm{m/s^2}$ otrzymano dla niemal wszystkich rozwiązań, dla których częstotliwość graniczna $f_{200,1}^{max}$ została przekroczona dla częstotliwości drgań własnych o postaci z podwójną półfalą.
...
\begin{enumerate}
	...
	\item Korelacja pomiędzy wysokością ściągu, a częstotliwością drgań własnych o postaci z podwójną półfalą istnieje, ale jest znacznie słabsza niż w przypadku wieszaków prostych (rys. \ref{fig:opti_res_2_freqs_netw_160_c}).
...
\begin{enumerate}
	...
	\item Wszystkie rozwiązania mieszczą się powyżej granicy $f_{200,1}^{max}$ dla częstotliwości o postaci z podwójną półfalą.  Przekłada się to na konieczność zastosowania dzielnika prędkości krytycznej $k>1$ dla tej postaci (rys. \ref{fig:opti_res_2_freqs_netw_200_h}). W obszarze otrzymanych rezultatów dla postaci z pojedynczą półfalą pojawia częstotliwość graniczna $f_{200,1}^{max}$. Do momentu jej uzyskania przez rozwiązania, decydująca jest postać o jednej półfali. Po jej przekroczeniu pojawia się pole gdzie decydująca jest postać drgań z podwójną półfalą.
