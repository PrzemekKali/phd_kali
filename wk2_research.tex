...
\end{equation}
gdzie $\text{MAC}(\cdot)$ oznacza kryterium MAC dwóch wektorów. Zasadniczo kryterium MAC dwóch wektorów powinno być maksymalizowane. Wszakże, dla idealnego dopasowania wektorów \higg{wartość kryterium równa jest jedności}, a dla zupełnego braku dopasowania - zeru. Jednakże, znając wartość maksymalną kryterium $\text{MAC}=1$ i stosując odejmowanie, przemianowano naturalnie maksymalizowaną funkcję celu na minimalizowaną. Dzięki temu wszystkie funkcje celu będą podlegać minimalizacji, co nie jest wymagane, ale uprasza interpretację wyników oraz zmniejsza stopień komplikacji algorytmu obliczeniowego.
...
\begin{enumerate}
	...
	\item Generacja obciążenia pociągiem HSLM-A. Analiza dynamiczna przejazdu pociągu po obiekcie. Odczytanie przebiegu przyspieszeń w czasie dla trzech punków zlokalizowanych na dźwigarze-ściągu w odległościach $1/4L$, $1/2L$ i $3/4L$ od podpory. \higg{Krok} ten wykonywany jest dla wszystkich dziesięciu pociągów HSLM-A i wyznaczonych prędkości. Łącznie około 30 analiz dynamicznych.
	...
	\item Odczytanie \higg{masy} konstrukcji stalowej na podstawie przekrojów poprzecznych łuku, ściągu i wieszaków.
...
Zastosowanie wieszaków typu Network zamiast wieszaków prostych z reguły jest rozwiązaniem gwarantującym większą sztywność konstrukcji, zwłaszcza przy niesymetrycznych obciążeniach. Intuicyjnie wydaje się, że układ Network powinien gwarantować również większą odporność konstrukcji łukowej na wzbudzenia dynamiczne. Tymczasem rezultaty analiz optymalizacyjnych przy przejazdach z maksymalną prędkością równą 300 km/h pokazują, że minimalne masy dźwigarów łukowych dla których spełniony jest warunek maksymalnych przyspieszeń wynoszą dla wariantów z wieszakami prostymi i Network odpowiednio 132 t i 150 t. Można by przypuszczać, że zmiana jedynie typu wieszaków dla rozwiązania z wieszakami prostymi na wieszaki Network powinna przynieść jeszcze lepszą odporność dynamiczną. Na początku oceniono czy różnica między masą rozwiązań nie wynika bezpośrednio z różnej masy wieszaków.  W tabeli \ref{tab:summary_mass_comparison} zestawiono masy poszczególnych elementów konstrukcyjnych dla wszystkich \enquote{najlepszych} rozwiązań przy prędkości maksymalnej 300 km/h (\ref{tab:summary_optimal_results}). Można dostrzec, że różnica w masie całkowitej pomiędzy wariantami z wieszakami prostymi i Network, przy tej samej prędkości maksymalnej jest znacznie większa od masy samych wieszaków w każdym wariancie. Wynika stąd, że wariant z wieszakami prostymi po zmianie na wieszaki Network nie powinien spełnić warunków poprawności rozwiązania. W przeciwnym razie ujawniłby się jako rozwiązanie lepsze od wyznaczonych dla wariantu Network rozwiązań optymalnych. Dokonano eksperymentu i sprawdzono maksymalne przyspieszania dla \enquote{najlepszego} rozwiązania z wieszakami prostymi, jedynie zamieniając wieszaki na wariant Network. Maksymalne przyspieszenia pomostu wyniosły w tym przypadku ponad 4.00 $\text{m}/\text{s}^2$.
...
Poszukiwania przyczyn zaistniałego dysonansu przeniesiono do dziedziny częstotliwości drgań własnych układu. Dla "najlepszego" optymalnego zestawu wymiarów elementów konstrukcyjnych wariantu z wieszakami prostymi i przy prędkości maksymalnej 300 km/h (Tab. \ref{tab:summary_optimal_results}) przeprowadzono eksperyment polegający na zamianie wieszaków na system Network i uzmiennieniu sztywności wieszaków. Sztywność wieszaków ograniczona jest jedynie do sztywności podłużnej zdefiniowanej iloczynem modułu sprężystości i pola przekroju wieszaków ($EA$). Nie chcąc uwzględniać zmiany masy wieszaków w prowadzonej analizie przyjęto stałą średnicę wieszaków jak w "najlepszym" rozwiązaniu. Jako zmienną przyjęto moduł sprężystości materiału wieszaków. Dla każdego przypadku sztywności wieszaków wyznaczono przyspieszenia maksymalne zgodnie z algorytmem opisanym w punkcie \ref{sect:optimization_assumptions}. Obliczono również częstotliwości drgań własnych sześciu pierwszych częstotliwości drgań własnych układu. Wyniki zaprezentowano w zależności od ekwiwalentnej średnicy określającej pole przekroju kołowego tak, aby wartość sztywności $EA$ w danym punkcie pozostała niezmieniona, a jednocześnie odpowiadała modułowi sprężystości stali $E=210 \mathrm{GPa}$. \higr{WNIOSEK?}

\begin{figure}[hbt!]
	\centering
	\includegraphics{WK2/dyskusja/proste_netw_change_stiff.pdf}
	\captionsetup{justification=centering}
	\caption{}
	\label{fig:proste_change_hangers}
\end{figure}
