...
W celu łatwiejszego odbioru dalszych rozważań na wstępie przedstawiono klasyfikację mostów łukowych. Tradycyjnie mosty łukowe wykonywane były jako pełne sklepienia, ze ściankami czołowymi i z wnęką wypełnioną piaskiem, żwirem lub słabym betonem. Ukształtowany w ten sposób ustrój mógł być wykonany z tradycyjnych materiałów takich jak na przykład kamień \parencite{Szczygie1978}. Obecnie tego typu obiekty wykonuje się niezmiernie rzadko. Służyły one do przekraczania raczej niewielkich rozpiętości (do ok. 40 m) i w dzisiejszych czasach zostały wyparte głównie przez strunobetonowe konstrukcje prefabrykowane \parencite{Ciesla2013} i obiekty gruntowo-powłokowe \parencite{Janusz2009,Tomala2019}. Obecnie mosty łukowe wykonuje się głównie w formie konstrukcji belkowo-płytowych. Ze względu na wzajemne usytuowanie pomostu i dźwigarów głównych można podzielić je następująco:
\begin{itemize}
	\item pomost usytuowany w całości nad łukiem - z tzw. "jazdą górą",
	\item pomost usytuowany w całości poniżej łuku - z tzw. "jazdą dołem",
	\item pomost usytuowany pośrednio - z tzw. "jazdą pośrednią".
\end{itemize}
Połączenie pomiędzy pomostem, a dźwigarem łukowym realizuje się za pomocą ścian lub słupków kiedy pomost znajduje się ponad dźwigarem. Kiedy pomost znajduje się poniżej stosowane są wieszaki. Tradycyjnie łączniki w obu wariantach są ukształtowane jako pionowe. W przypadku konstrukcji z jazdą górą odstępstwa od tej zasady zastosował między innymi Morandi (rys. ). W przypadku obiektów z jazdą gorą występują trzy główne schematy rozkładu wieszaków:
\begin{itemize}
...
\begin{itemize}
	...
	\item drastyczne zmniejszenie długości wyboczeniowej łuku w płaszczyźnie wieszaków.
\end{itemize}
W ocenie 
...
Mosty łukowe wykonywane są obecnie z dwóch materiałów: stali i betonu (w tym betonu sprężonego). Zależnie od materiału dźwigar łukowy może posiadać różną strukturę. Najczęściej łuki występują w trzech formach \
...
\begin{figure}[h]
	\centering
	\subfloat[wieszki proste]{\includegraphics[width=0.7\linewidth]{/mosty_wstep/mosty-proste.pdf}}  \\
	\subfloat[wieszaki ukośne]{\includegraphics[width=0.7\linewidth]{/mosty_wstep/mosty-ukosne.pdf}} \\
	\subfloat[wieszaki typu Network]{\includegraphics[width=0.7\linewidth]{/mosty_wstep/mosty-network.pdf}}
