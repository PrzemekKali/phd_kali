...
\subsection{Funkcje korelacji - odpowiedź swobodna układu}
Funkcje korelacji posiadają dwie wspomniane wcześniej właściwości kluczowe dla OMA. Po pierwsze teoretycznie zawierają wszystkie informacje na temat parametrów modalnych konstrukcji. Po drugie mogą być utożsamiane z drganiami swobodnymi, gasnącymi układu \parencite{James1995}.

Zwykle zakładamy, że mierzone sygnały posiadają wartość średnią równą zero oraz są Gaussowskie \teng{Gaussian signals} lub bliskie Gaussowskim \parencite{Brincker2015}. W takich przypadkach wszystkie parametry modalne możemy odnaleźć w funkcjach korelacji. Możemy to stwierdzić na bazie centralnego twierdzenia granicznego \teng{central limit theorem}. Mówi ono, że dla niezależnych zmiennych losowych $\bm{X_i}$ o jednakowym rozkładzie, fluktuujących wokół wartości oczekiwanej $\mu$ i o skończonej wariancji $\sigma^2$ to wyrażenie \ref{eq:centralLimitTheorem}
\begin{equation} \label{eq:centralLimitTheorem}
	\frac{\frac{1}{n}\sum_{i=1}^{M} \bm{X_i} - \mu}{\frac{\sigma}{M}}
\end{equation}
