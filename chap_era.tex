...
\end{equation}
Wykorzystanie właściwości splotu funkcji korelacji (!!!) i  splotu\footnote{Transformata Fouriera splotu dwóch funkcji w dziedzinie czasu $h(t)$ i $g(t)$ jest równa iloczynowi transformat Fouriera każdej z funkcji osobno. Innymi słowy transformacie Fouriera wyrażenia $h(t)*g(t)$ odpowiada iloczyn $H_k G_k$, gdzie: $H_k$ - transformata Fouriera funkcji $h(t)$, $G_k$ - transformata Fouriera funkcji $g(t)$.} oraz transformaty Fouriera pozwala uzyskać następującą właściwość gęstości widmowej:
\begin{equation} 
	G_{xy}(\omega) = X^*(\omega)Y(\omega)
\end{equation}

Sprawdź symetrię Hermitowską
https://pl.wikipedia.org/wiki/Widmo_sygna%C5%82u
